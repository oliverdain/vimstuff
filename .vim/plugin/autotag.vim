        ...
        #self.diag("does %s exist?", tagsFile)
         if os.path.isfile(tagsFile):
           #self.diag("%s DOES exist!", tagsFile)
            return tagsFile
			# Modified by Oliver on 10/15/2008. The original code, commented out
			# below, searched all the way up to the root dir. The problem was it
			# searched for /home/tags which is an automount dir which could cause
			# really long hangs. So now it stops at my home dir.
			# elif not file or file == os.sep:
         elif not file or file == os.sep or file == os.environ['HOME']:
